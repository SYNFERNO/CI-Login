...
class Auth extends CI_Controller
{
    ...
    public function registration()
    {
        ...
        $this->form_validation->set_rules('email', 'Email', 'required|trim|valid_email|is_unique[user.email]', [
            'is_unique' => 'This email has alerady registered!'
        ]);
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->session->set_flashdata(
                'message',
                '<div class="alert alert-success" role="alert">Congratulation! your account has been created. Please Activate your account!</div>'
            );
            redirect('auth');
        }
    }
        ...
        if ($user) {
            ...
            if ($user_token) {
                if (time() - $user_token['date_created'] < (60 * 60 * 24)) {
                    ...
                    $this->session->set_flashdata(
                        'message',
                        '<div class="alert alert-success" role="alert">' . $email . ' has been activate!</div>'
                    );
                    redirect('auth');
                } else {
    ...
    function _login()
    {
        ...
        if ($user) {
            if ($user['is_active'] == 1) {
                ...
                $this->session->set_flashdata(
                    'message',
                    '<div class="alert alert-danger" role="alert">Email is not been activated!</div>'
                );
                redirect('auth');
            }
        } else {
    ...
    public function changePassword()
    {
        ...
        $this->form_validation->set_rules('password2', 'Retype Password', 'trim|required|min_length[8]|matches[password1]');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->session->set_flashdata(
                'message',
                '<div class="alert alert-success" role="alert">Reset password success! Please Login.</div>'
            );
            redirect('auth');
        }
    }
